...
Run:  python tests/test_composer.py   (from the Promptforge-revised folder)

Also cross-checks the Python subset compiler against the canonical JS
compiler via Node, when node is available on PATH.
"""
...
PKG = os.path.join(HERE, "..", "prompt-bridge-forge", "__init__.py")

spec = importlib.util.spec_from_file_location("prompt_bridge_forge", PKG)
...
composer = mod.PromptForgeComposer()
bridge = mod.PromptForgeBridge()


@test("bridge node behavior is unchanged")
def _():
...
def _():
    a = mod.PromptForgeComposer.IS_CHANGED("s", "p", "n", "prepend", "", "")
    b = mod.PromptForgeComposer.IS_CHANGED("s", "p", "n", "prepend", "", "")
    c = mod.PromptForgeComposer.IS_CHANGED("s", "p2", "n", "prepend", "", "")
    assert a == b and a != c
...
def _():
    ...
    js_dir = os.path.join(HERE, "..", "prompt-bridge-forge", "js").replace("\\", "/")
    script = f"""
      const fs = require('fs');
    ...
    py_out = mod.compile_state(state)
